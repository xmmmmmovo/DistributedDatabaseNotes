...
const static char *PD_LOG_HEADER_FORMAT =
    "%04d-%02d-%02d-%02d.%02d.%02d.%06d\
               \
Level:%s" OSS_NEWLINE "PID:%-37dTID:%d" OSS_NEWLINE
...
static int _pdLogFileWrite(const char *pData) {
    ...
    if (rc) {
        printf(
            "Failed to write into log file, errno = %d" OSS_NEWLINE,
            rc);
        goto error;
    }
done:
...
void pdLog(PDLEVEL level, const char *func, const char *file,
    ...
    if (_pdDiagLogPath[0] != '\0') {
        ...
        if (rc) {
            printf("Failed to write into log file, errno = "
                   "%d" OSS_NEWLINE,
                   rc);
            printf("%s" OSS_NEWLINE, sysInfo);
        }
