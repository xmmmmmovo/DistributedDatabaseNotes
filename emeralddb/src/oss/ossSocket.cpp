...
_ossSocket::_ossSocket(int *sock, int timeout) {
    ...
    if (rc) {
        ...
        PD_RC_CHECK(rc, PDERROR,
                    "Failed to get peer name, error = %d",
                    SOCKET_GETLASTERROR);
    }
done:
    return;
...
int _ossSocket::bind_listen() {
    ...
    if (rc) {
        PD_RC_CHECK(EDB_NETWORK, PDERROR,
                    "Failed to bind socket, rc = %d",
                    SOCKET_GETLASTERROR);
    }
    rc = listen(_fd, SOMAXCONN);
    if (rc) {
        PD_RC_CHECK(EDB_NETWORK, PDERROR,
                    "Failed to listen socket, rc = %d",
                    SOCKET_GETLASTERROR);
    }
done:
    return rc;
...
int _ossSocket::send(const char *pMsg, int len, int timeout,
    ...
    while (len > 0) {
        // MSG_NOSIGNAL: Requests not to send SIGPIPE on errors on
        // stream oriented sockets when the other end breaks the
...
int _ossSocket::recvNF(char *pMsg, int len, int timeout) {
    ...
    while (true) {
        ...
        // if the socket we interested is not receiving anything,
        // let's continue
        if (FD_ISSET(_fd, &fds)) {
            break;
        }
...
int _ossSocket::connect() {
    int rc = EDB_OK;
    rc =
        ::connect(_fd, (struct sockaddr *)&_sockAddress, _addressLen);
    if (rc) {
        PD_RC_CHECK(EDB_NETWORK, PDERROR,
                    "Failed to connect, rc = %d",
                    SOCKET_GETLASTERROR);
    }
    rc = getsockname(_fd, (sockaddr *)&_sockAddress, &_addressLen);
    if (rc) {
    ...
    // get peer address
    rc =
        getpeername(_fd, (sockaddr *)&_peerAddress, &_peerAddressLen);
    if (rc) {
        PD_RC_CHECK(EDB_NETWORK, PDERROR,
...
int _ossSocket::accept(int *sock, struct sockaddr *addr,
...
int _ossSocket::_getAddress(sockaddr_in *addr, char *pAddress,
                            unsigned int length) {
    int rc = EDB_OK;
    length = length < NI_MAXHOST ? length : NI_MAXHOST;
    rc     = getnameinfo((struct sockaddr *)addr, sizeof(sockaddr),
                     pAddress, length, NULL, 0, NI_NUMERICHOST);
    if (rc) {
        PD_RC_CHECK(EDB_NETWORK, PDERROR,
                    "Failed to getnameinfo, rc = %d",
                    SOCKET_GETLASTERROR);
    }
done:
    return rc;
...
int _ossSocket::getPort(const char *    pServiceName,
                        unsigned short &port) {
    int             rc = EDB_OK;
