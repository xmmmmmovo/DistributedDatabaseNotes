...
int ossPrimitiveFileOp::Open(const char * pFilePath,
...
int ossPrimitiveFileOp::Read(const size_t size, void *const pBuffer,
...
int ossPrimitiveFileOp::Write(const void *pBuffer, size_t size) {
    ...
    if (isValid()) {
        do {
            rc = oss_write(_fileHandle,
                           &((char *)pBuffer)[currentSize],
                           size - currentSize);
            if (rc >= 0)
                currentSize += rc;
